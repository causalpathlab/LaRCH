...
from nn.base_components import BeyesianETMEncoder, BeyesianETMDecoder, deltaTopicEncoder, deltaTopicDecoder, ETMDecoder
...
class ETM_module(BaseModuleClass):
    ...
    def __init__(
        ...
        self.decoder = ETMDecoder(self.n_latent, 
                                  self.n_input,         
                                )
    ...
    def generative(self, z) -> dict:
        ...
        rho, theta  = self.decoder(z)

        return dict(rho = rho, theta = theta)
    ...
    def loss(
        ...
        return LossRecorder(loss, reconstruction_loss, kl_local,
                            ...
                            kl_beta = torch.Tensor(0), 
                            kl_rho = torch.Tensor(0), 
...
class deltaTopic_module(BaseModuleClass):
    ...
    def __init__(
        ...
        n_genes: int,
        n_latent: int = 32,
        n_layers_encoder_individual: int = 3,
        dim_hidden_encoder: int = 128,
        pip0_rho: float = 0.1,
    ...
    def _get_inference_input(self, tensors):
        return dict(x=tensors[_CONSTANTS.X_KEY],y = tensors[_CONSTANTS.UNSPLICED_EXP_KEY])
    ...
    def inference(self, x: torch.Tensor, y: torch.Tensor) -> dict:
        ...
        z, z_x, z_y, q_x_m, q_x_v, q_y_m, q_y_v = self.z_encoder(x_, y_)
        return dict(z = z, z_x = z_x, z_y = z_y, q_x_m = q_x_m, q_x_v = q_x_v, q_y_m = q_y_m, q_y_v = q_y_v)
    ...
    def sample_from_posterior_z(
        ...
        if deterministic: # average of the two means WITHOUT sampling
            z_x = inference_out["q_x_m"]
            z_y = inference_out["q_y_m"]
            z = (z_x + z_y)/2
        else: # sampling 
            z = inference_out["z"]
        if output_softmax_z:
    ...
    def loss(
        ...
        kl_weight_beta = self.kl_weight_beta
        x = tensors[_CONSTANTS.X_KEY]
        y = tensors[_CONSTANTS.UNSPLICED_EXP_KEY]
        q_x_m = inference_outputs["q_x_m"]
        q_x_v = inference_outputs["q_x_v"]
        q_y_m = inference_outputs["q_y_m"]
        q_y_v = inference_outputs["q_y_v"]
        ...
        # KL Divergence for z [batch_size]
        mean = torch.zeros_like(q_x_m)
        scale = torch.ones_like(q_x_v)
        kl_divergence_x = kl(Normal(q_x_m, torch.sqrt(q_x_v)), Normal(mean, scale)).sum(
            dim=1
        )
        kl_divergence_y = kl(Normal(q_y_m, torch.sqrt(q_y_v)), Normal(mean, scale)).sum(
            dim=1
        )
        # suming over all the topics
        # kl_divergence for beta, rho_kl, tensor of torch.size([]) <- torch.sum([N_topics, N_genes])
        kl_divergence_beta = rho_kl + delta_kl
        kl_local = kl_divergence_x + kl_divergence_y
