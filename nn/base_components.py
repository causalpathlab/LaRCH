...
class deltaTopicEncoder(nn.Module):
    """
    latent VAE encoder with two input heads --> one shared latent space by mixture of experts
    """
    def __init__(
        self,
        n_input_list: List[int],
        ...
        self.mean_encoder_x = nn.Linear(n_hidden, n_output)
        self.var_encoder_x = nn.Linear(n_hidden, n_output)
        
        self.mean_encoder_y = nn.Linear(n_hidden, n_output)
        self.var_encoder_y = nn.Linear(n_hidden, n_output)
    
    def forward(self, x: torch.Tensor, y: torch.Tensor, *cat_list: int):
        #x: spliced; #y: unspliced
        ...
        q_x_m = self.mean_encoder_x(q_x)
        q_x_v = torch.exp(torch.clamp(self.var_encoder_x(q_x), -4.0, 4.0)/2.)
        latent_x = reparameterize_gaussian(q_x_m, q_x_v)
        
        q_y_m = self.mean_encoder_y(q_y)
        q_y_v = torch.exp(torch.clamp(self.var_encoder_y(q_y), -4.0, 4.0)/2.)
        latent_y = reparameterize_gaussian(q_y_m, q_y_v)
         
        # mixture of experts
        latent = (latent_x + latent_y)/2
        
        return latent, latent_x, latent_y, q_x_m, q_x_v, q_y_m, q_y_v
...
class ETMDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.rho = nn.Parameter(torch.randn(n_input, n_output), requires_grad = True)
        self.log_softmax = nn.LogSoftmax(dim=-1)
