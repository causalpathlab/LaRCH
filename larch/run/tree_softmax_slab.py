...
from larch.util.modelhub import TreeSoftmaxSlab
...
def main():
    ...
    model_id = f"tree_softmax_slab_{args.data_id}_ep{args.EPOCHS}_treeD{args.tree_depth}_bs{args.bs}_lr{args.lr}_train_size{args.train_size}_pip{args.pip0}_kl{args.kl_weight}_klbeta{args.kl_weight_beta}_seed{args.seed}"
    print(model_id)
    ...
    if os.path.exists(os.path.join(args.out_dir, model_id)):
        ...
        model = TreeSoftmaxSlab(adata, tree_depth = args.tree_depth, alpha0_rho=args.pip0, kl_weight_beta = args.kl_weight_beta, kl_weight = args.kl_weight)
